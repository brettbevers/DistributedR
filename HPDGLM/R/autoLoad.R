...
dataLoader <- function(tableName, resp = list(...), pred = list(...),conf) {
    ...
    X <- darray(dim=c(nobs, nPredictors), blocks=c(rowsInBlock,nPredictors))
    #Binary response vector: value one shows sucess and zero failure 
    Y <- darray(dim=c(nobs,nResponses), blocks=c(rowsInBlock,nResponses))
...
# Example:
# loadedData <- dataLoader("mortgage", list("def"), list("mltvspline1", "mltvspline2", "agespline1", "agespline2", "hpichgspline", "ficospline"),conf="RDev")
